...
# PROGRAM_PATHS = ['exec/algo-nw-impl1', 'exec/algo-nw-impl2', 'exec/algo-nw-impl1-openmp', 'exec/algo-nw-impl2-openmp']
# PROGRAM_PATHS = ['exec/algo-nw-ref']
PROGRAM_PATHS = ['exec/algo-nw-target']
...
if __name__ == '__main__':
    ...
    for prog_path in PROGRAM_PATHS:
        with open(f'{OUTPUT_FILENAME}_{prog_path[prog_path.rfind("/") + 1:]}.tsv', mode='w') as f:
            for fn1, fn2 in zip(sorted(os.listdir(SEQUENCE1_PATH), key=lambda x: (len(x), x))[:10],
                                sorted(os.listdir(SEQUENCE2_PATH), key=lambda x: (len(x), x))[:10]):
